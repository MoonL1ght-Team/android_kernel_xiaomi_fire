...
#ifndef get_shipmode_status
static inline int __fire_compat_get_shipmode_status(void)
{
	return 0;
}
#define get_shipmode_status() __fire_compat_get_shipmode_status()
#endif

/*
 * 6.6 port compatibility.
 *
 * The original 4.19 driver used platform helpers that are not exported in this
 * tree. Keep the charger core functional and make the optional hooks harmless.
 */
#ifndef HARDWARE_CHARGER_IC
#define HARDWARE_CHARGER_IC 0
#endif

#ifndef SYV690_COMPAT_HELPERS
#define SYV690_COMPAT_HELPERS

static inline int syv690_compat_battery_get_vbus(void)
{
	/*
	 * Old MTK battery_get_vbus() is not available here.
	 * Returning 0 keeps optional HVDCP/PD reset paths from making unsafe
	 * assumptions before a proper 6.6 VBUS provider is wired.
	 */
	return 0;
}

static inline int syv690_compat_adapter_set_cap(int mv, int ma)
{
	/*
	 * Old global adapter_set_cap() is not available in this 6.6 tree.
	 * Do not fail charger registration because of an optional PD reset hook.
	 */
	return 0;
}

static inline int syv690_compat_hardwareinfo_set_prop(int prop, const char *val)
{
	return 0;
}

#define battery_get_vbus() syv690_compat_battery_get_vbus()
#define adapter_set_cap(mv, ma) syv690_compat_adapter_set_cap((mv), (ma))
#define hardwareinfo_set_prop(prop, val) syv690_compat_hardwareinfo_set_prop((prop), (val))

#ifndef EVENT_EOC
#define EVENT_EOC 0
#endif

#ifndef KERNEL_POWER_OFF_CHARGING_BOOT
#define KERNEL_POWER_OFF_CHARGING_BOOT 8
#endif

#ifndef LOW_POWER_OFF_CHARGING_BOOT
#define LOW_POWER_OFF_CHARGING_BOOT 9
#endif

static inline bool syv690_compat_charger_manager_pd_is_online(void)
{
	return false;
}

static inline bool syv690_compat_is_usb_rdy(void)
{
	return true;
}

#define charger_manager_pd_is_online() syv690_compat_charger_manager_pd_is_online()
#define is_usb_rdy() syv690_compat_is_usb_rdy()


#endif /* SYV690_COMPAT_HELPERS */
...
#define SYV690_KPOC_BOOT 8
#define SYV690_LOW_POWER_OFF_CHARGING_BOOT 9
...
struct SYV690_device {
	...
	bool attach;
	bool tcpc_attach;
	enum syv690_charger_type chg_type;
	bool bc12_en;
	bool lk_type_is_cdp;
	bool kpoc;
...
int SYV690_enable_term(struct SYV690_device *chgInfo, bool enable);
...
*/
int SYV690_process_safety_timer(struct SYV690_device *chgInfo ,bool enable)
{
	int ret = 0;
...
#endif
static int __maybe_unused SYV690_chgdet_pre_process(struct SYV690_device *chgInfo)
{
	int ret = 0;
	bool attach = false;
...
static void SYV690_remove(struct i2c_client *client)
{
}
